...
function Header({ updateRecipeData }) {
    ...
    const handleBiryani = async () => {
        ...
        try {
            setBLoading(true);
          const response = await fetch(`https://indian-food-9wno.onrender.com/api/v1/biryani?srno=573,631,881,1793,3703,4959`);
          if (!response.ok) {
      ...
      const handleVeg = async () => {
        ...
        try {
            setVLoading(true);
          const response = await fetch(`https://indian-food-9wno.onrender.com/api/v1/veg?srno=273,715,1863,2563,1594,6060,3018`);
          if (!response.ok) {
            throw new Error('Failed to fetch data');
          }
          const responseData = await response.json();
